...
if (is_numeric($text)&&($text<=$max)&&($text>=4) )
{
  ...
  $wtarget=$main."subjectview4.php?which=".$text."'>"; 
  echo $wtarget; 
}
else
//{if (!is_numeric($text))
{
 ...
 echo "<strong>Must be a valid request number. Try Again... </strong><br>"; 
 if (($text>$max)||($text<33))
 {echo "Must be a valid request number between 4-".$max.". Try again...";}

 $main="<META HTTP-EQUIV='refresh' content='5;URL=";
  $wtarget=$main."Key.php"."'>"; 
  echo $wtarget;}
  ?>
